...
const ViolationRoutes = {
  BlockingBikeLane: {
    name: 'Blocking Bike Lane',
  },
  BlockingHydrant: {
    name: 'Blocking Hydrant',
  },
  BlockingSidewalk: {
    name: 'Blocking Sidewalk',
  },
  BusLayover: {
    name: 'Bus Layover',
  },
  CommercialOvernight: {
    name: 'Commercial Overnight',
  },
  DoubleParked: {
    name: 'Double Parked',
  },
...
class IllegalParkingScreen extends React.Component {
  constructor(props) {
    super(props);
  ...
  _saveDetails = () => {
    console.log('submit report triggered for Illegal Parking');
  ...
  _getValue = (value, type) => {
    if (type === 'violation') {
      Object.keys(Routes).map((routeName: string, index) => (
        Routes['IllegalParkingViolationScreen'].isSet = true
      ));
      Object.keys(Routes).map((routeName: string) => (
        Routes['IllegalParkingViolationScreen'].value = value
      ));
      this.setState({parkingViolation: value});
    }
    this.props.navigation.goBack(null);
  };
